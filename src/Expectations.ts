...
⬥ Execution of the ${Emojis.decimation} rotation is expected.\n
...
const createEmbed = (data: string, colour: number) => {
    return {
        color: colour,
        description: data
    };
...
const startingEmbed = createEmbed(generalExpectations, Colours.gold);
const magicEntEmbed = createEmbed(magicEnt, Colours.blue);
const meleeEntEmbed = createEmbed(meleeEnt, Colours.red);
const rangedEntEmbed = createEmbed(rangedEnt, Colours.green);
const hybridMageRangedEntEmbed = createEmbed(hybridMageRangedEnt, Colours.aqua);
const chinnerEmbed = createEmbed(chinner, Colours.green);
const freeEmbed = createEmbed(free, Colours.gold);
const baseEmbed = createEmbed(base, Colours.blue);
const otherEmbed = createEmbed(other, Colours.gold);
...
client.on("ready", async () => {
    ...
    client.channels.fetch(Channels.performanceExpectations)
        .then(async channel => {
            ...
            await (channel as TextChannel).send(`> __**General Expectations**__`).then(message => {
                createTableOfContentsItem(message, 'General');
            });
            await (channel as TextChannel).send({ embeds: [startingEmbed] });
            await (channel as TextChannel).send(`> __**Magic Entangle**__`).then(message => {
                createTableOfContentsItem(message, 'Magic Entangle');
            });
            await (channel as TextChannel).send({ embeds: [magicEntEmbed] });
            await (channel as TextChannel).send(`> __**Melee Entangle**__`).then(message => {
                createTableOfContentsItem(message, 'Melee Entangle');
            });
            await (channel as TextChannel).send({ embeds: [meleeEntEmbed] });
            await (channel as TextChannel).send(`> __**Ranged Entangle**__`).then(message => {
                createTableOfContentsItem(message, 'Ranged Entangle');
            });
            await (channel as TextChannel).send({ embeds: [rangedEntEmbed] });
            await (channel as TextChannel).send(`> __**Magic/Ranged Entangle**__`).then(message => {
                createTableOfContentsItem(message, 'Magic/Ranged Entangle');
            });
            await (channel as TextChannel).send({ embeds: [hybridMageRangedEntEmbed] });
            await (channel as TextChannel).send(`> __**Chinner**__`).then(message => {
                createTableOfContentsItem(message, 'Chinner');
            });
            await (channel as TextChannel).send({ embeds: [chinnerEmbed] });
            await (channel as TextChannel).send(`> __**Free**__`).then(message => {
                createTableOfContentsItem(message, 'Free');
            });
            await (channel as TextChannel).send({ embeds: [freeEmbed] });
            await (channel as TextChannel).send(`> __**Base**__`).then(message => {
                createTableOfContentsItem(message, 'Base');
            });
            await (channel as TextChannel).send({ embeds: [baseEmbed] });
            await (channel as TextChannel).send(`> __**Other Information**__`).then(message => {
                createTableOfContentsItem(message, 'Other');
            });
            await (channel as TextChannel).send({ embeds: [otherEmbed] });
