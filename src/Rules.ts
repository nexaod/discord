...
⬥ All splits should be calculated using either the **-splitnexaod** command in Discord, or the Alt1 **BetterAoD** calculator.\n
...
const startingEmbed = createEmbed(generalRules, Colours.gold);
const behaviouralEmbed = createEmbed(behavioural, Colours.gold);
const reportingEmbed = createEmbed(reporting, Colours.red);
const teamFormingEmbed = createEmbed(teamForming, Colours.gold);
const splittingEmbed = createEmbed(splitting, Colours.gold);
const deathsEmbed = createEmbed(deaths, Colours.gold);
const gemsEmbed = createEmbed(gems, Colours.gold);
const adminEmbed = createEmbed(admin, Colours.gold);
...
client.on("ready", async () => {
    ...
    client.channels.fetch(Channels.rules)
        .then(async channel => {
            ...
            await (channel as TextChannel).send(`> __**General Rules**__`).then(message => {
                createTableOfContentsItem(message, 'General');
            });
            await (channel as TextChannel).send({ embeds: [startingEmbed] });
            await (channel as TextChannel).send(`> __**Behavioural Rules**__`).then(message => {
                createTableOfContentsItem(message, 'Behavioural');
            });
            await (channel as TextChannel).send({ embeds: [behaviouralEmbed] });
            await (channel as TextChannel).send(`> __**Reporting Players**__`).then(message => {
                createTableOfContentsItem(message, 'Reports');
            });
            await (channel as TextChannel).send({ embeds: [reportingEmbed] });
            await (channel as TextChannel).send(`> __**Team Forming**__`).then(message => {
                createTableOfContentsItem(message, 'Team Forming');
            });
            await (channel as TextChannel).send({ embeds: [teamFormingEmbed] });
            await (channel as TextChannel).send(`> __**Splitting**__`).then(message => {
                createTableOfContentsItem(message, 'Splitting');
            });
            await (channel as TextChannel).send({ embeds: [splittingEmbed] });
            await (channel as TextChannel).send(`> __**Deaths**__`).then(message => {
                createTableOfContentsItem(message, 'Deaths');
            });
            await (channel as TextChannel).send({ embeds: [deathsEmbed] });
            await (channel as TextChannel).send(`> __**Challenge Gems**__`).then(message => {
                createTableOfContentsItem(message, 'Gemming');
            });
            await (channel as TextChannel).send({ embeds: [gemsEmbed] });
            await (channel as TextChannel).send(`> __**Administration Team**__`).then(message => {
                createTableOfContentsItem(message, 'Administration Team');
            });
            await (channel as TextChannel).send({ embeds: [adminEmbed] });
